...
double positionValue(Piece pc, int8 pos)
{
  ...
  if (pc == B)
    ...
    posVal = abs(3.5 - row); // want the king to be in the centre
...
double gameOver(const Board &board)
{
  ...
  if (!black)
    return -1; // no more black pieces

  return 1; // no more white pieces
}
...
double staticEval(const Board &board, bool player)
{
  ...
  for (int i = 0; i < bSize; i++)
  {
    Piece pc = board.get(i);
    if (pc != E)
      numpcs++; // counts number of pieces
  }
  for (int i = 0; i < bSize; i++)
  {
    Piece pc = board.get(i);
    ...
    if (numpcs < 12)
    {
      ...
      if (pc == WK || pc == BK)
        ...
        value *= positionValue(pc, i); // pawns will try harder to get kings
    }
    eval += value;
  }
