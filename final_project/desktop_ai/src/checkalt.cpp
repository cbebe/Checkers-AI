                ...
                tile_t neighbour, dir_t direction, Piece query)
{
  ...
  if (neighbour == ADJACENT)
    rowOS(pos, offset); // adjacent offsets depend on piece's position
  else
    diagOS(offset); // diagonal offsets are constant in every direction

  Piece queryK = E; // assume query is E

  if (query != E)
    queryK = (query == B) ? BK : WK; // assigning the query king piece
...
moveList captureBothDirs(const Board &board, int8 pos)
{
  ...
  for (int i = 0; i < 4; i++)
    if (empties[i] != -1 && enemies[i] != -1)
      captures.push_back({pos, empties[i]}); // move can be made
...
moveList get::moves(const Board &board, int8 pos)
{
  ...
  if (pcMvs.p1 != -1)
    moves.push_back({pos, pcMvs.p1});
  if (pcMvs.p2 != -1)
    moves.push_back({pos, pcMvs.p2});
...
void checkForChain(bList &boardList, moveList &captures, const Board &board)
{
  for (auto capture : captures)
  {
    ...
    if (pieceCaps.empty())
      boardList.push_back(bcopy); // no chains; evaluation is complete
    else
      checkForChain(boardList, pieceCaps, bcopy); // recursive call
  }
}
...
void check::captures(bList &boardList, const Board &board, bool player)
{
  ...
  for (int i = 0; i < bSize; i++)
  {
    ...
    if (p == piece || p == pieceK)
    {
      ...
      if (!pieceCaps.empty())
        checkForChain(boardList, pieceCaps, board); // now we're checking for capture chains
    }
  }
}

// moves the pieces on the board according to the move-position pair
